...
2. **Geolocation, server-side.** `detect_from_geolocation()` queues whatever it finds, and `prime_currency_cookie()` — hooked at `template_redirect`, priority 0 — writes it to the cookie, **unless the current render is one a page cache is about to store.** On a cacheable render (cache compatibility mode on, visitor logged out, any front-end page other than the cart, checkout or account page) this write is deliberately skipped: writing it there would bake one visitor's geolocated currency into the cached HTML and hand it to everyone who loads that page afterward.
